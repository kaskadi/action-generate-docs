...
function processFunction (layersMeta) {
  return lambda => {
    ...
    return {
      name,
      sources: events.length > 0 ? `<ul style="margin: 0;">${events.map(event => `<li>${getEventName(event)}</li>`).join('')}</ul>` : 'No source defined',
      timeout: timeout || 'default',
      handler,
      ...layers && { layers: `<ul style="margin: 0;">${layers.map(layer => `<li>${getLayerName(layer, layersMeta)}</li>`).join('')}</ul>` },
      ...destinations && { destinations: `<ul style="margin: 0;"><li>On success: ${destinations.onSuccess}</li><li>On failure: ${destinations.onFailure}</li></ul>` }
    }
  }
}
